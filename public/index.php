...
if ($authenticated && $csrfOK && $method === 'POST' && ($_POST['action'] ?? '') === 'save_company') {
    ...
    $pixKey = PixPayload::normalizeKey((string) ($companyInput['pix_key'] ?? ''));
    if ($pixKey === null) {
        $error = 'A chave Pix não é válida. Use CPF/CNPJ, e-mail, chave aleatória ou telefone com DDD.';
    } else {
        $companyInput['pix_key'] = $pixKey;
    }
    $logo = $_FILES['company_logo'] ?? null;
    if (!$error && $logo && $logo['error'] !== UPLOAD_ERR_NO_FILE) {
...
if ($page === 'invoice' && $data) {
    ...
    $rawPixKey = (string) ($company['pix_key'] ?? '');
    $pixPayload = PixPayload::build($rawPixKey, (float) $calculation['amount_due'], (string) ($company['trade_name'] ?? ''), (string) ($company['pix_city'] ?? ''));
    if ($pixPayload !== null) {
        $pixQrCode = pixQrCode($pixPayload);
    } elseif (trim($rawPixKey) !== '') {
        $pixError = 'A chave Pix cadastrada não está em um formato válido. Atualize-a em Dados da Gestora.';
    }
}
?>
<!doctype html>
...
<head>
  ...
  <style>
    .system-mark{display:grid;place-items:center;width:58px;height:58px;border-radius:14px;background:var(--green);color:#fff;font-size:29px;font-weight:700}.home-brand{text-decoration:none;color:inherit;width:max-content}.home-brand:hover{color:#126747}.invoice-branding .brand{gap:0;font-size:27px}.invoice-branding .brand::before{display:none}.invoice-branding p{line-height:1.35}.invoice-branding p small{color:var(--muted);font-size:12px}.manager-identity{display:flex;align-items:center;gap:10px;margin:0 0 9px}.manager-identity .manager-logo{display:block;max-width:120px;max-height:42px;object-fit:contain;object-position:left center;flex:0 0 auto}.manager-identity strong{margin:0}.party-card .manager-identity+p{margin-top:0}.customer-form input[name="amount_due"],.customer-form input[name="savings_amount"]{background:#f4faf7;color:#668078;pointer-events:none}.dashboard{margin-bottom:22px}.dashboard h1{margin-bottom:4px}.dashboard .summary{grid-template-columns:repeat(4,1fr);margin:18px 0}.dashboard .chart{height:250px;background:#fff}.dashboard-note{color:var(--muted);font-size:13px;margin:8px 0 0}.pix-payment{display:flex;align-items:center;gap:18px;margin-top:18px;padding:14px;border:1px solid #b8d5c9;border-radius:12px;background:#f8fcfa}.pix-payment img{width:150px;height:150px;image-rendering:pixelated;background:#fff;padding:5px}.pix-payment p{margin:4px 0}.pix-copy{font-size:11px;overflow-wrap:anywhere;color:var(--muted)}@media(max-width:700px){.dashboard .summary{grid-template-columns:1fr 1fr}.pix-payment{align-items:flex-start;flex-direction:column}}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}.invoice-branding .brand{display:flex!important;font-size:22px}.system-mark{width:45px;height:45px;font-size:23px}.manager-identity{gap:7px;margin-bottom:5px}.manager-identity .manager-logo{max-width:95px;max-height:32px}.pix-payment{break-inside:avoid;page-break-inside:avoid}.pix-payment img{width:112px;height:112px}}
  </style>
</head>
<body><main class="wrap">
  ...
  <?php elseif ($page === 'invoice' && $data && $calculation): ?>
    ...
    <article class="invoice"><header class="invoice-top"><div class="invoice-branding"><div class="system-mark" aria-hidden="true">☀</div><div><div class="brand">SolarFatura</div><p>Fatura inteligente de energia compensada<br><small>Versão <?= escape($appVersion) ?> (software livre)</small></p></div></div><div class="invoice-meta"><small>Referência</small><strong><?= escape($data['reference_month'] ?? '') ?></strong><small>Vencimento</small><strong><?= escape($data['due_date'] ?? '') ?></strong></div></header>
      ...
      <h2>Memória de cálculo</h2><table class="table"><tr><td>Energia compensada com desconto de <?= escape($data['discount_percent']) ?>%</td><td><?= money($calculation['solar_energy']) ?></td></tr><tr><td><?= escape($data['availability_label'] ?? 'Disponibilidade') ?></td><td><?= money($data['availability_amount']) ?></td></tr><tr><td>Iluminação pública</td><td><?= money($data['public_lighting']) ?></td></tr><tr><td>Acerto anterior</td><td><?= money($data['adjustment_amount']) ?></td></tr><tr><td>Bônus</td><td>- <?= money($data['bonus_amount']) ?></td></tr></table><div class="total"><span>Total a pagar</span><span><?= money($calculation['amount_due']) ?></span></div><?php if ($pixQrCode): ?><section class="pix-payment"><img src="<?= escape($pixQrCode) ?>" alt="QR Code Pix para pagar <?= money($calculation['amount_due']) ?>"><div><strong>Pague com Pix</strong><p>Aponte a câmera para o QR Code. O valor de <b><?= money($calculation['amount_due']) ?></b> já será preenchido no aplicativo do banco.</p><p class="pix-copy">Código Pix: <?= escape($pixPayload) ?></p></div></section><?php elseif ($pixError): ?><p class="warning"><?= escape($pixError) ?></p><?php else: ?><p class="warning">Cadastre a chave Pix em Dados da Gestora para incluir o QR Code de pagamento na fatura.</p><?php endif; ?><p><strong>Simulação sem energia fotovoltaica:</strong> <?= money($calculation['without_solar']) ?> · <strong>Economia:</strong> <?= money($calculation['savings']) ?></p>
